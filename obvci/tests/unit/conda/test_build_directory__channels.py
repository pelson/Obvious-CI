...
from obvci.conda_tools.inspect_binstar import distribution_exists, distribution_exists_on_channel, add_distribution_to_channel
def test_distribution_exists():
    clear_binstar(CLIENT, OWNER)

    # Build a recipe.
    meta = MetaData(os.path.abspath('../recipes/recipe1_dev'))
    meta = build(meta)
...
def test_leaky_add_to_channel():
    ...
    # Build a recipe and upload the recipe to the testing channel.
    meta_eariler = MetaData(os.path.abspath('../recipes/recipes_directory/recipe1'))
...
from obvci.conda_tools.build_directory import recipes_to_build, fetch_metas
def test_recipes_to_build():
    clear_binstar(CLIENT, OWNER)

    # Build a recipe.
    meta = build(MetaData('../recipes/recipes_directory/recipe1'))
    upload(CLIENT, meta, OWNER, channels=['testing'])

    metas = fetch_metas('../recipes/recipes_directory/')
...
from obvci.conda_tools.build_directory import fetch_metas, sort_dependency_order
